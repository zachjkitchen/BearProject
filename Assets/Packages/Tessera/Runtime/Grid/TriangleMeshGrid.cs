...
namespace Tessera
{
    ...
    /// </summary>
    internal class TriangleMeshGrid : IGrid
    {
        ...
        public TriangleMeshGrid(ICellType cellType, MeshData surfaceMesh, int layerCount, float tileHeight, float surfaceOffset, bool surfaceSmoothNormals, IDictionary<(Vector3Int, CellFaceDir), (Vector3Int, CellFaceDir, CellRotation)> moves)
        {
            ...
            this.moves = moves;
            this.faceCounts =  Enumerable.Range(0, surfaceMesh.subMeshCount).Select(i => surfaceMesh.indices[i].Length / 3).ToArray();
            this.maxFaceCount = faceCounts.Max();
            this.subMeshCount = surfaceMesh.subMeshCount;
            this.indexCount = faceCounts.Sum() * layerCount;
        ...
        {
            // TODO: Better support for other offsets?
            ...
            foreach(var c in GetCellsIntersectsApprox(pb, false))
            {
                ...
                if (TrianglePrismGeometryUtils.FindCell(Vector3.zero, Vector3.one, tileCenter, m  * tileLocalToGridMatrix, out cell, out rotation) && cell == Vector3Int.zero)
                {
                    cell = c;
                    return true;
                }
        ...
        public bool FindCell(Vector3 position, out Vector3Int cell)
        {
            ...
            foreach (var c in GetCellsIntersectsApprox(pb, false))
            {
                var trs = GetTRS(c);
                var m = trs.ToMatrix().inverse;
                if (TrianglePrismGeometryUtils.FindCell(Vector3.zero, Vector3.one, m * position, out cell) && cell == Vector3Int.zero)
                {
                    cell = c;
                    return true;
                }
        ...
        {
            // Triangles are always points up, see MeshGridBuilder
        ...
        {
            // Because all triangles are consider to be points up
            // the rotations between them are always odd.
            yield return TriangleRotation.RotateCCW60(1);
            yield return TriangleRotation.RotateCCW60(3);
            yield return TriangleRotation.RotateCCW60(5);
            // Plus identity for vertical motion
        ...
        private MeshDetails BuildMeshDetails()
        {
            ...
            foreach (var cell in GetCells())
            {
                ...
                var dim = GeometryUtils.Abs(cellTrs.ToMatrix().MultiplyVector(Vector3.one * Mathf.Sqrt(3)));
