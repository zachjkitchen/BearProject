...
namespace Tessera
{
    public class CubeCellDrawingType : ICellDrawingType
    {
        ...
        public void GetFaceCenterAndNormal(Vector3Int offset, CellFaceDir dir, Vector3 center, Vector3 tileSize, out Vector3 faceCenter, out Vector3 faceNormal)
        {
            ...
            var forward = ((CubeFaceDir)dir).Forward();
        ...
        public RaycastCellHit Raycast(Ray ray, Vector3Int offset, Vector3 center, Vector3 tileSize, float? minDistance, float? maxDistance)
        {
            ...
            var currentHitDir = (CubeFaceDir)currentHitCellDir;
        ...
        private bool Raycast(Vector3 center, Vector3 hsize, Ray ray, out CellFaceDir faceDir, out Vector3 point)
        {
            ...
            faceDir = tmin == t3.x ? (dir.x > 0 ? (CellFaceDir)CubeFaceDir.Left : (CellFaceDir)CubeFaceDir.Right)
                : tmin == t3.y ? (dir.y > 0 ? (CellFaceDir)CubeFaceDir.Down : (CellFaceDir)CubeFaceDir.Up)
                : (dir.z > 0 ? (CellFaceDir)CubeFaceDir.Back : (CellFaceDir)CubeFaceDir.Forward);
            return true;
        }
        ...
        {
            return offset + ((CubeFaceDir)dir).Forward();
        }
